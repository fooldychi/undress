...
async function createWorkflowConfigTable() {
  try {
    console.log('🔧 开始创建工作流配置表...');
    ...
    // 验证插入结果
    const workflowInfoResult = await query('SELECT * FROM workflow_info ORDER BY workflow_type');
    console.log('\n📊 工作流信息:');
    workflowInfoResult.forEach(info => {
      console.log(`  ${info.workflow_type}: ${info.workflow_name} (${info.is_enabled ? '启用' : '禁用'})`);
    });

    const nodeConfigResult = await query(`
...
module.exports = { createWorkflowConfigTable };
